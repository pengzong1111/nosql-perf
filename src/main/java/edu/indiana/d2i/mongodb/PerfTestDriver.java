...
public class PerfTestDriver {
	...
	public static void main(String[] args) throws InterruptedException, FileNotFoundException {
		...
		if(Configuration.getProperty("ACCESS_PATTERN").equals("VOLUME")) {
			pw = new PrintWriter(new FileOutputStream("overall-stats-mongo.txt", true));
		} else {
			pw = new PrintWriter(new FileOutputStream("overall-page-stats-mongo.txt", true));
		}
		...
		for(int i=0; i<threadNum; i++) {
			...
			/*if(Configuration.getProperty("ACCESS_PATTERN").equals("VOLUME")) {
				retriever = new MongoDBVolumeRetriever(i);
			} else {
				retriever = new MongoDBPageRetriever(i);
			}*/
			retriever = new MongoDBRetriever(i);
